...
import { mkdirpObservable, readFileObservable, writeFileObservable } from '../utils/fs';
...
const notebooks = argv._.filter(x => /(.ipynb)$/.test(x)).filter(x => existsSync(resolve(x)));
...
export function createSplashSubscriber() {
  ...
  return Rx.Subscriber.create(
    () => {
      ...
      const index = join(__dirname, '..', '..', 'lib', 'app', 'splash.html');
...
windowAllClosed.skipUntil(appAndKernelSpecsReady).subscribe(closeAppOnNonDarwin);

const openFile$ = Rx.Observable.fromEvent(app, 'open-file', (event, filename) => ({
  event,
  filename,
}));
